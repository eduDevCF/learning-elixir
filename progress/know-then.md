# Elixir and Phoenix For Beginners
Course: (https://knowthen.com/category/elixir-and-phoenix-for-beginners)
By: James Moore

- [x] Introduction
- [x] Installing Elixir
- [ ] ▶️ [Data Types](#data-types)
- [ ]  Pattern Matching
- [ ]  Elixir Source Files
- [ ]  Control Flow
- [ ]  Functions
- [ ]  Collections
- [ ]  Protocols
- [ ]  Macros
- [ ]  Build Tool - Mix
- [ ]  What is Functional Programming?
- [ ]  First Elixir Application
- [ ]  Processes
- [ ]  OTP
- [ ]  Phoenix Web Framework
- [ ]  Exit Polling Application
- [ ]  Poller Dal
- [ ]  Poller Phoenix
- [ ]  Final Thoughts

## Introduction
- [x] Who is this course for
- [x] Why was Elixir Created?
- [x] Thinking about concurrency
- [x] Concurrency in Sequential Languages

## Installing Elixir
- [x] Installing Elixir
- [x] Installing Elixir on Mac OS
- [x] Installing Elixir on Windows
- [x] Installing Elixir on Ubuntu
- [x] What we installed

## Data Types
- [x] Integers
- [x] Floats
- [x] Atoms
- [x] Boolean
- [x] Strings
- [x] [Maps](https://knowthen.com/elixir-and-phoenix-for-beginners/data-types/maps)
- [ ] [Lists](https://knowthen.com/elixir-and-phoenix-for-beginners/data-types/lists)
- [ ] List Operations
- [ ] Tuples
- [ ] Keyword Lists
- [ ] Structs
- [ ] Immutable Data
- [ ] Variables

## Pattern Matching
- [ ] Pattern Matching Introduction
- [ ] Pattern Matching Strings
- [ ] Pattern Matching Maps
- [ ] Pattern Matching Structs
- [ ] Pattern Matching Lists
- [ ] Pattern Matching Tuples

## Elixir Source Files
- [ ] Elixir Source Files

## Control Flow
- [ ] Control Flow Introduction
- [ ] Control Flow Cond
- [ ] Control Flow Case
- [ ] Control Flow Unless

## Functions
- [ ] Anonymous Functions
- [ ] Named Functions
- [ ] Function Pattern Matching
- [ ] Function & Module Docs
- [ ] Private Functions
- [ ] Anonymous Function Pattern Matching
- [ ] Anonymous Function Dot Syntax
- [ ] Higher Order Functions
- [ ] Higher Order Functions Continued
- [ ] Odds & Ends
- [ ] Odds & Ends Review
- [ ] Guards
- [ ] Sigils
- [ ] Pure vs. Impure Functions
- [ ] Pipe Operator
- [ ] Recursion
- [ ] Tail Call Optimization

## Collections
- [ ] Collection Processing
- [ ] Mapping
- [ ] Filter
- [ ] Reduce
- [ ] Composition With Pipe (|>)
- [ ] Stream
- [ ] Comprehensions

## Protocols
- [ ] Protocols

## Macros
- [ ] Macros Introduction
- [ ] Quote & Unquote
- [ ] Macros Example
- [ ] Using Macros

## Build Tool - Mix
- [ ] Mix

## What is Functional Programming?
- [ ] What is Functional Programming

## First Elixir Application
- [ ] First Application Overview
- [ ] Building Apps Thought Process
- [ ] Mix Project and Data Structures
- [ ] First Application View Functions
- [ ] First Application View Body Functions
- [ ] Review Challenge
- [ ] Update Functions
- [ ] Update Functions - Add Candidate
- [ ] Update Functions - Vote Command
- [ ] Tie things together
- [ ] Testing
- [ ] Summarize First Application

## Processes
- [ ] Process Introduction
- [ ] What is a process
- [ ] Process Lifetime
- [ ] Process Links
- [ ] Long Lived Processes
- [ ] Improving the counter
- [ ] Error Handling
- [ ] Supervisor Introduction
- [ ] Basic HTTP Server
- [ ] Create A Supervisor
- [ ] The Actor Model

## OTP
- [ ] Introduction to OTP
- [ ] Introduction to Genserver
- [ ] Genserver Callbacks
- [ ] A More Idiomatic GenServer
- [ ] OTP Supervisor
- [ ] Cache Genserver
- [ ] Application Behavior
- [ ] Dynamic Supervisor
- [ ] Agents & Tasks

## Phoenix Web Framework
- [ ] Introduction to the Phoenix Web Framework
- [ ] Installing Phoenix
- [ ] Phoenix Overview
- [ ] First Phoenix Application
- [ ] Adding A Page
- [ ] Databases & Phoenix
- [ ] Review Generated Files

## Exit Polling Application
- [ ] Planning
- [ ] Start Poller Project
- [ ] Poller Project Data
- [ ] Poller Project Functions
- [ ] Try Out The Functions & Structs
- [ ] Poll GenServer
- [ ] Try Out Pollserver GenServer
- [ ] Create Supervisor For Pollserver
- [ ] Poller Application Behavior

## Poller Dal
- [ ] Introduction
- [ ] Ecto Introduction
- [ ] Getting Started With Poller Dal
- [ ] Adding A Repo
- [ ] Add A Table
- [ ] Changesets
- [ ] Database CRUD API
- [ ] Questions Table And Functions
- [ ] Choices Table And Functions
- [ ] Users
- [ ] Hash User Password
- [ ] User API

## Poller Phoenix
- [ ] Phoenix Project Introduction
- [ ] Districts
- [ ] Create Districts
- [ ] Edit Districts
- [ ] Delete Districts
- [ ] Question Pages
- [ ] Add Questions
- [ ] Edit & Delete Questions
- [ ] Choices Pages
- [ ] Review Choice Pages Solution
- [ ] Authentication & Access Control
- [ ] Login Form
- [ ] Auth Plugs
- [ ] NavBar Links
- [ ] Rest API
- [ ] Voting Endpoint
- [ ] Initialize Poll GenServer
- [ ] Save Votes to the Database
- [ ] Connect the React App
- [ ] Phoenix Channels Introduction
- [ ] Phoenix Channels
- [ ] Channel Callbacks
- [ ] PubSub
- [ ] Integrate PubSub

## Final Thoughts
- [ ] Final Thoughts